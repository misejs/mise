...
var createViewmodel = function(name,noRoute,callback,force){
  ...
  var routesFile = path.join(miseDir,config.paths.clientRoutes);

  var viewmodelRoutesPath = utils.requireString(path.relative(path.dirname(routesFile),viewmodelFile));
  ...
  async.waterfall([
    ...
    utils.updateFile.bind(utils,viewmodelFile,/{{name}}/g,utils.capitalize(name)),
    createRoute
  ],callback);
};
