...
var utils = module.exports = {
  ...
  runCmd : function(destinationPath,command,callback){
    if(typeof command == 'function'){
      callback = command;
      command = destinationPath;
      destinationPath = utils.miseDir();
      if(!destinationPath) return callback(new Error('Unable to find .mise directory.'));
    }
    var cwd = process.cwd();
    var child = exec('cd '+destinationPath+' && '+command+' && cd '+cwd, function (error, stdout, stderr) {
      callback(error);
    });
    // child.stderr.pipe(process.stderr);
    child.stdout.pipe(process.stdout);
  },
  runGulp : function(task,callback){
    console.log('running '+task+'...');
    utils.runCmd('gulp ' + task,callback);
  },
  npmInstall : function(destinationPath,callback){
    console.log('installing dependencies...');
    if(typeof destinationPath == 'function'){
      utils.runCmd('npm install',destinationPath);
    } else {
      utils.runCmd(destinationPath,'npm install',callback);
    }
  },
  ...
  updateFile : function(path,find,replace,allowFailure,callback){
    ...
    fs.readFile(path,'utf-8',function(err,contents){
      ...
      if(!matches && !allowFailure){
        callback(new Error('Unable to update file '+path+', no matches found for '+find+'.'));
      } else {
